...
@keyframes waveHand{0%{transform:rotate(0)}10%{transform:rotate(14deg)}20%{transform:rotate(-8deg)}30%{transform:rotate(14deg)}40%{transform:rotate(-4deg)}50%{transform:rotate(10deg)}60%,100%{transform:rotate(0)}}
</style>
...
<header class="fixed top-0 left-0 right-0 md:left-60 h-16 md:h-20
  ...
  <div class="flex items-center gap-3 w-full md:w-auto">
    <button id="sidebarToggle" type="button" class="md:hidden text-gray-700 dark:text-gray-200">
      ☰
    </button>
  ...
  <div class="flex items-center gap-4 md:gap-6 text-sm
    ...
    <form method="POST" action="set-currency.php">
      <select name="currency"
              ...
              class="bg-transparent cursor-pointer text-gray-700 dark:text-gray-200">
        <?php foreach ($CURRENCIES as $code => $c): ?>
          <option value="<?= htmlspecialchars($code) ?>" <?= (currency() === $code) ? 'selected' : '' ?>>
